...
func main() {
	...
	// Example: Add some products (only if they don't already exist)
	fmt.Println("\n🔍 Adding products (checking for duplicates)...")

	err = addProductIfNotExists(db, "Mechanical split keyboard", 279.99)
	if err != nil {
...
func displayMenu() {
	...
	fmt.Println("3. 🗑️  Delete a product")
	fmt.Println("4. 🚪 Exit")
	fmt.Println(strings.Repeat("=", 50))
...
func runInteractiveMenu(db *sql.DB) {
	for {
		...
		if err != nil {
			fmt.Printf("❌ Invalid input. Please enter a number between 1 and 4.\n")
		...
		case 4:
			...
			return
		default:
			fmt.Printf("❌ Invalid option: %d. Please choose between 1 and 4.\n", choice)
		}
